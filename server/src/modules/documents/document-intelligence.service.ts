...
export class DocumentIntelligenceService {
  ...
  private pickBestBusinessNameLine(text: string) {
    ...
    return (
      lines.find((line) =>
        /\b(LTD|LIMITED|PLC|LLC|INC|COMPANY)\b\.?$/i.test(line),
      ) ??
      lines.find((line) =>
        /\b(LTD|LIMITED|PLC|LLC|INC|COMPANY)\b/i.test(line),
      ) ??
  ...
  private extractCacRegistrationNumber(text: string) {
    ...
    const singleLetterMatch = text.match(
      /\bR\s*[-:]?\s*([0-9][0-9,\s.-]{2,})\b/i,
    );
    ...
    if (labelledMatch) {
      const cleanedValue = labelledMatch[1]
        .replace(/[\s,.-]+/g, '')
        .toUpperCase();
  ...
  private extractIdentityDocumentNumber(text: string) {
    ...
    const genericMatch = text.match(
      /\b([A-Z]{1,3}[0-9]{6,12}|[0-9]{10,14})\b/i,
    );
