    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 shrink-0 space-y-2">
          <button
            onClick={() => setParams("?tab=api")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${active === "api" ? "bg-white/10 text-white border border-white/10" : "text-gray-400 hover:text-white hover:bg-white/5 border border-transparent"}`}
          >
            <Key
              className={`h-4 w-4 ${active === "api" ? "text-emerald-400" : ""}`}
            />
            API & Webhooks
          </button>
          <button
            onClick={() => setParams("?tab=profile")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${active === "profile" ? "bg-white/10 text-white border border-white/10" : "text-gray-400 hover:text-white hover:bg-white/5 border border-transparent"}`}
          >
            <User
              className={`h-4 w-4 ${active === "profile" ? "text-emerald-400" : ""}`}
            />
            Profile
          </button>
          <button
            onClick={() => setParams("?tab=team")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${active === "team" ? "bg-white/10 text-white border border-white/10" : "text-gray-400 hover:text-white hover:bg-white/5 border border-transparent"}`}
          >
            <Shield
              className={`h-4 w-4 ${active === "team" ? "text-emerald-400" : ""}`}
            />
            Team & Security
          </button>
          <button
            onClick={() => setParams("?tab=notifications")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${active === "notifications" ? "bg-white/10 text-white border border-white/10" : "text-gray-400 hover:text-white hover:bg-white/5 border border-transparent"}`}
          >
            <Bell
              className={`h-4 w-4 ${active === "notifications" ? "text-emerald-400" : ""}`}
            />
            Notifications
          </button>
        </div>
            ...
            <>
              <div className="glass-panel rounded-3xl p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="border-b border-white/10 pb-6 mb-6">
                  ...
                  <p className="text-sm text-gray-400">
                    Use these keys to authenticate API requests and integrate
                    VeriSphere with your backend systems.
                  </p>
                </div>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      Only reveal this key to secure backend servers. Never
              ...
              <div className="glass-panel rounded-3xl p-8 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-100">
                <div className="border-b border-white/10 pb-6 mb-6">
                  ...
                  <p className="text-sm text-gray-400">
                    Configure where VeriSphere should send real-time updates
                    about vendor verification status.
                  </p>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      Use this secret to verify that webhook payloads actually
                      came from VeriSphere.
                    </p>
                  </div>
            ...
            <div className="glass-panel rounded-3xl p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-6">
                  <div className="h-24 w-24 rounded-full bg-linear-to-tr from-emerald-500 to-cyan-500 border-4 border-gray-900 shadow-[0_0_15px_rgba(16,185,129,0.4)] flex items-center justify-center text-3xl font-black text-gray-950">
                    JD
                  </div>
                ...
                {teamMembers.map((member, i) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="h-10 w-10 rounded-full bg-linear-to-br from-gray-700 to-gray-900 flex items-center justify-center font-bold text-white">
                      ...
                      <div>
                        <p className="text-sm font-bold text-white">
                          ...
                          {member.status === "Pending" && (
                            <span className="ml-2 text-[10px] bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded border border-amber-500/30">
                              Pending
                            </span>
                          )}
                        </p>
                    ...
                    <div className="flex items-center gap-4">
                      <span className="text-xs font-semibold text-gray-300 bg-white/10 px-3 py-1 rounded-lg">
                        {member.role}
                      </span>
                ...
                <div className="flex items-center justify-between p-5 bg-black/30 rounded-xl border border-white/5">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Force all members to enable 2FA before accessing the
                      console.
                    </p>
                  </div>
          ...
          {active === "notifications" && (
            <div className="glass-panel rounded-3xl p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-4">
                {[
                  {
                    title: "High-Risk Vendor Applications",
                    desc: "Get an email immediately when a vendor is flagged as high-risk.",
                    active: true,
                  },
                  {
                    title: "Squad Payment Failures",
                    desc: "Receive alerts when verification fees fail to process.",
                    active: true,
                  },
                  {
                    title: "Webhook Delivery Errors",
                    desc: "Get notified if we cannot reach your configured webhook endpoint.",
                    active: true,
                  },
                  {
                    title: "Weekly Summary Report",
                    desc: "A digest of vendor screenings and trust scores every Monday.",
                    active: false,
                  },
                  {
                    title: "New Device Sign-ins",
                    desc: "Alerts for unrecognized devices accessing your console account.",
                    active: true,
                  },
                ].map((notif, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-4 p-4 hover:bg-white/5 rounded-xl transition-colors"
                  >
                    <button
                      className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border transition-colors ${notif.active ? "bg-emerald-500 border-emerald-500 text-gray-950" : "border-gray-600 bg-black/40"}`}
                    >
