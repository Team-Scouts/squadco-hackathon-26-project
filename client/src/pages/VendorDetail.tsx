...
const graphNodes: GraphNode[] = [
  { id: "vendor", label: "Northline", type: "Vendor", x: 50, y: 50, tone: "vendor" },
  { id: "device", label: "iPhone 14", type: "Device", x: 24, y: 30, tone: "risk" },
  { id: "bank", label: "Acct 0192", type: "Bank", x: 78, y: 36, tone: "warn" },
  { id: "doc", label: "CAC hash", type: "Document", x: 74, y: 70, tone: "risk" },
  { id: "ip", label: "IP cluster", type: "Network", x: 30, y: 76, tone: "neutral" },
...
function getNodeTone(tone: GraphNode["tone"]) {
  const tones = {
    vendor: "bg-cyan-500/20 border-cyan-400/60 text-cyan-300 shadow-[0_0_24px_rgba(34,211,238,0.28)]",
...
function DocumentModification() {
  ...
  const updateField = (label: string, value: string) => {
    setFields((current) =>
      current.map((field) =>
        field.label === label
          ? { ...field, verified: value, status: value === field.extracted ? "match" : "edited" }
          : field,
      ),
    );
  };
      ...
      <div className="flex flex-col gap-4 border-b border-white/10 pb-5 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-white">Document Modification</h2>
          </div>
          <p className="mt-1 text-sm text-gray-400">
            Correct OCR fields, compare document evidence, and preserve the review history.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold text-gray-300 transition-colors hover:bg-white/10 hover:text-white">
          ...
          {documents.map((document) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`rounded border px-2 py-0.5 text-[10px] font-bold ${document.color}`}>
                  {document.status}
                </span>
              </div>
              <p className="mt-3 text-sm font-bold text-white">{document.name}</p>
              <p className="mt-1 text-xs text-gray-500 font-mono">{document.id}</p>
              ...
              {fields.map((field) => (
                <div key={field.label} className="rounded-xl border border-white/10 bg-black/30 p-4">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    ...
                    {field.status === "match" && <CheckCircle2 className="h-5 w-5 text-emerald-400" />}
                    {field.status === "edited" && <Edit3 className="h-5 w-5 text-cyan-400" />}
                    {field.status === "flagged" && <AlertTriangle className="h-5 w-5 text-amber-400" />}
                  </div>
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    <label>
                      ...
                      <input readOnly value={field.extracted} className={`${fieldInput} text-gray-400`} />
                    </label>
                    <label>
                      <span className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-gray-500">
                        Verified
...
export default function VendorDetail() {
  ...
  const metrics = [
    { label: "Trust score", value: "24", icon: ShieldAlert, color: "text-red-400" },
    { label: "Documents", value: "7", icon: FileText, color: "text-emerald-400" },
    { label: "Graph links", value: "18", icon: GitBranch, color: "text-cyan-400" },
    { label: "Open alerts", value: "5", icon: AlertTriangle, color: "text-amber-400" },
  ];

  const timeline = [
    ["Document hash collision", "CAC image matched Koro Market Services", "12m ago", "critical"],
    ["Reviewer edited field", "Business name normalized after OCR check", "18m ago", "info"],
    ["Squad fee verified", "sq_txn_9x2b4 confirmed NGN 15,000", "24m ago", "safe"],
    ["Device cluster detected", "6 applications from same fingerprint", "41m ago", "critical"],
  ];
      ...
      <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-gray-400">
            Individual vendor review workspace for trust scoring, document correction,
            entity graph analysis, Squad payment telemetry, and final reviewer decision.
          </p>
        </div>
        ...
        <aside className="space-y-6">
          <section className="glass-panel rounded-2xl p-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-bold text-white">Northline Exports Ltd</p>
                <p className="text-xs text-gray-500">Supplier - Lagos, Nigeria</p>
              </div>
            </div>
            ...
            <div className="mt-5 space-y-3">
              {(
                ...
                <div key={String(value)} className="flex items-center gap-3 rounded-xl border border-white/5 bg-black/30 p-3">
          ...
          <section className="glass-panel rounded-2xl p-6">
            ...
            <div className="mt-4 space-y-3">
              {(
                ...
                <div key={String(label)} className="flex items-center justify-between rounded-xl border border-white/10 bg-black/30 p-3">
                  <div className="flex items-center gap-3">
                    <Icon className={`h-4 w-4 ${complete ? "text-emerald-400" : "text-gray-500"}`} />
                    <span className="text-sm font-semibold text-gray-300">{String(label)}</span>
                  </div>
                  <span className={`h-5 w-5 rounded-full border ${complete ? "border-emerald-500 bg-emerald-500 text-gray-950" : "border-white/20"}`}>
              ...
              {timeline.map(([title, detail, time, tone]) => (
                <div key={title} className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[10px] font-mono text-gray-600">{time}</p>
                  </div>
                </div>
              ))}
            </div>
