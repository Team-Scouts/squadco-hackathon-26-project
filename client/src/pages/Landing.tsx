...
const steps = [
  {
    title: "INTAKE",
    text: "Capture vendor identity, business details, device consent, and uploaded documents.",
  },
  {
    title: "VERIFY",
    text: "Initiate a Squad verification payment with vendor and risk-session metadata attached.",
  },
  {
    title: "ANALYZE",
    text: "Ingest webhook events, write relationships, and refresh document, device, network, and payment scores.",
  },
  {
    title: "DECIDE",
    text: "Route each case to approve, review, or reject with evidence an operator can defend.",
  },
];
...
const cases = [
  {
    ...
    status: "CLEAR",
    score: "84",
    signal: "Clean document hash, verified payment, no shared account history.",
    color: "#22c55e",
  },
  {
...
export default function Landing() {
  ...
  return (
    ...
    >
      ...
      {/* ━━ HEADER ━━ */}
      <header
        className={`fixed inset-x-0 top-0 z-50 flex min-h-16 items-center justify-between gap-6 px-6 md:px-12 transition-all duration-500 ${scrollY > 50 ? "bg-[#0a0a0f]/90 backdrop-blur-xl border-b border-white/10" : "bg-transparent"}`}
      >
        <a
        ...
        >
          {["Intelligence", "Workflow", "Cases"].map((item) => (
            <a
              key={item}
              className="inline-flex min-h-9 items-center rounded px-4 text-xs font-bold uppercase tracking-[0.2em] text-gray-500 no-underline transition-all hover:text-white hover:bg-white/5 font-mono"
      ...
      <main id="top" className="relative z-10">
          ...
          <div className="w-full max-w-4xl mx-auto relative">
            ...
            <div className="mb-8 inline-flex items-center gap-3 rounded border border-sky-300/20 bg-sky-300/5 px-4 py-2 font-mono text-xs text-sky-100 animate-fade-in-up backdrop-blur-sm">
              <span className="flex h-2 w-2">
                <span className="absolute inline-flex h-2 w-2 animate-ping rounded-full bg-sky-300 opacity-60" />
                <span className="relative inline-flex h-2 w-2 rounded-full bg-sky-300" />
              </span>
              <span className="opacity-60">[SYS]</span> AI Trust Graph for
              Verified Actors
            </div>

            {/* Main heading with glitch */}
            <h1
              className="m-0 text-5xl font-black leading-[1.05] tracking-tight sm:text-7xl lg:text-8xl animate-fade-in-up"
              style={{ animationDelay: "200ms", animationFillMode: "both" }}
            >
              <span className="block text-white/90 animate-glitch-subtle">
                FRAUD
              </span>
              <span className="block text-sky-200">LENS</span>
            </h1>

            {/* Subtitle with typing */}
            <div
              className="mt-8 max-w-2xl font-mono text-sm md:text-base leading-relaxed text-gray-500 animate-fade-in-up h-14"
            ...
            >
              ...
              <a
                className="inline-flex min-h-12 items-center justify-center rounded border border-violet-300/20 bg-violet-300/5 px-8 text-sm font-bold uppercase tracking-[0.15em] text-violet-100/80 no-underline font-mono transition-all hover:border-violet-300/35 hover:text-violet-100 hover:bg-violet-300/10 hover:-translate-y-0.5 backdrop-blur-sm"
                href="#workflow"
              >
                See_Workflow
              </a>
          ...
          <div className="mt-auto pt-16 w-full max-w-4xl mx-auto">
            ...
            >
              {metrics.map((m, i) => (
                <div
                  key={m.label}
                  className="group rounded border border-white/5 bg-white/[0.02] p-5 backdrop-blur-sm transition-all duration-500 hover:border-white/20 hover:bg-white/[0.045]"
                ...
                >
                  <div className="flex items-center gap-2 text-xs font-mono text-gray-600 uppercase tracking-wider">
                    <span style={{ color: metricAccent(m.label) }}>
                      {m.icon}
                    </span>{" "}
                    {m.label}
                  </div>
        ...
        {/* ━━ INTELLIGENCE ━━ */}
        <section
          className="relative border-t border-white/10 bg-black/20 px-6 py-24 md:px-12 md:py-32"
          id="intelligence"
        >
          ...
          <div className="mx-auto mb-16 w-full max-w-4xl">
            <p className="mb-4 text-xs font-bold uppercase tracking-[0.3em] text-sky-200 font-mono">
              <span className="opacity-50">[</span>Live Verification Console
            ...
            <p className="mt-5 max-w-2xl text-sm leading-7 text-gray-500">
              FraudLens consolidates the signals reviewers need before approving
              a vendor: identity, documents, devices, payment proof, and graph
              relationships.
            </p>
          </div>
          ...
          <div className="mx-auto w-full max-w-5xl rounded border border-sky-300/15 bg-[#0d0d14]/85 backdrop-blur-xl overflow-hidden animate-neon-pulse-blue shadow-[0_24px_90px_rgba(0,0,0,0.45),0_0_70px_rgba(56,189,248,0.08)]">
            ...
            <div className="flex items-center gap-2 px-5 py-3 bg-black/40 border-b border-white/5">
              ...
              <span className="ml-3 text-[10px] font-mono text-gray-600 uppercase tracking-wider">
                fraudlens://risk-console
              </span>
            </div>
            ...
            <div className="p-6 md:p-8">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-5 border-b border-white/5 pb-6">
                <div>
                  ...
                  <h3 className="mt-2 text-xl font-black text-white font-mono">
              ...
              <div className="mt-6 grid gap-6 sm:grid-cols-[auto_1fr]">
                ...
                <div className="relative grid h-36 w-36 shrink-0 place-items-center mx-auto sm:mx-0">
                  <svg
                    className="absolute inset-0 h-full w-full -rotate-90 animate-rotate-slow"
                  ...
                  <div className="text-center relative z-10">
                    <span className="block text-4xl font-black leading-none text-white font-mono text-glow">
                      59
                    </span>
                    <small className="mt-1 block text-[10px] font-bold uppercase tracking-wider text-gray-500 font-mono">
                      Trust Score
                    </small>
                  </div>
                </div>
                ...
                <div className="grid gap-2">
                  {[
                    ["Shared device cluster", "HIGH", "#ff006e"],
                    ["Payment authenticity", "MED", "#f59e0b"],
                    ["Document integrity", "LOW", "#22c55e"],
                  ].map(([label, value, color]) => (
                    <div
                      className="flex justify-between items-center gap-4 rounded border border-white/5 bg-white/[0.02] p-3 transition-colors hover:bg-white/[0.05] group"
                      key={label}
                    >
          ...
          <div className="mx-auto mt-8 w-full max-w-5xl grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {modules.map((mod, i) => (
              <article
                key={mod.title}
                className="group relative overflow-hidden rounded border border-white/5 bg-white/[0.02] backdrop-blur-sm p-5 transition-all duration-500 hover:-translate-y-1 hover:bg-white/[0.04]"
                style={{
                  animationDelay: `${i * 100}ms`,
                  borderColor: `${mod.color}00`,
                  transition: "all 0.5s",
                }}
                onMouseEnter={(e) =>
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span
                    className="flex h-9 w-9 shrink-0 items-center justify-center rounded font-mono font-black text-xs"
                    style={{
                      color: mod.color,
        ...
        {/* ━━ WORKFLOW ━━ */}
        <section
          className="relative border-t border-white/10 bg-[#08080d]/70 px-6 py-24 md:px-12 md:py-32"
          id="workflow"
        >
          ...
          <div className="mx-auto mb-16 w-full max-w-4xl">
            <p className="mb-4 text-xs font-bold uppercase tracking-[0.3em] text-violet-200 font-mono">
              <span className="opacity-50">[</span>Protocol Sequence
              <span className="opacity-50">]</span>
            </p>
            <h2 className="m-0 text-3xl font-black leading-tight tracking-tight text-white lg:text-5xl">
            ...
            <p className="mt-5 max-w-2xl text-sm leading-7 text-gray-500">
              Each stage writes durable evidence first, then updates derived
              intelligence layers so reviewers can trace the decision path.
            </p>
          </div>
          ...
          <div className="mx-auto grid w-full max-w-5xl gap-4 lg:grid-cols-4">
            {steps.map((step, i) => (
              <article
                key={step.title}
                className="group relative rounded border border-white/5 bg-white/[0.02] backdrop-blur-sm p-6 transition-all duration-500 hover:-translate-y-2 hover:border-white/20 hover:bg-white/[0.045] overflow-hidden"
              >
                {/* Step number */}
                ...
                <h3 className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-zinc-400 font-mono group-hover:text-white transition-colors">
                  {step.title}
                </h3>
        ...
        {/* ━━ CASES ━━ */}
        <section
          className="relative border-t border-white/10 bg-black/25 px-6 py-24 md:px-12 md:py-32"
          id="cases"
        >
          ...
          <div className="mx-auto mb-16 w-full max-w-4xl">
            <p className="mb-4 text-xs font-bold uppercase tracking-[0.3em] text-sky-200 font-mono">
              <span className="opacity-50">[</span>Operator View
              <span className="opacity-50">]</span>
            </p>
            <h2 className="m-0 text-3xl font-black leading-tight tracking-tight text-white lg:text-5xl">
            ...
            <p className="mt-5 max-w-2xl text-sm leading-7 text-gray-500">
              The operator view is built for fast triage: scores, status, and
              the primary evidence signal stay visible without opening a full
              case.
            </p>
          </div>
          ...
          <div className="mx-auto w-full max-w-5xl rounded border border-white/10 bg-[#0d0d14]/80 backdrop-blur-xl overflow-hidden">
            ...
            {cases.map((c) => (
              ...
              >
                <span className="font-bold text-white text-sm font-mono group-hover:text-zinc-200 transition-colors">
                  {c.name}
                </span>
                <span className="flex items-center">
                  <span
                    className="inline-flex items-center rounded px-3 py-1 text-[10px] font-bold uppercase tracking-[0.15em] font-mono"
                    style={{
                      color: c.color,
